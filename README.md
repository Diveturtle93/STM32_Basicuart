...
Die zu verwendende UART-Schnittstelle wird zur Laufzeit über `initBasicUart()` übergeben.
...
```c
void initBasicUart(UART_HandleTypeDef *huart);              // UART-Handle setzen
```
...
void uartTransmitUNumber(const uint32_t number, const uint32_t base);  // Zahl ohne Vorzeichen senden
...
|------------------------|----------------------------------|---------------------------------------------------------------------------|
| `initBasicUart`        | `*huart`                         | Speichert das UART-Handle für alle nachfolgenden Sende-/Empfangsfunktionen |
...
in `usart.c`. Das Handle wird anschließend einmalig an die Bibliothek übergeben,
typischerweise in `main()` nach `MX_USART2_UART_Init()`:

```c
initBasicUart(&huart2);
```

Es kann jedes beliebige UART-Handle übergeben werden – die Bibliothek ist nicht auf
eine bestimmte Schnittstelle festgelegt.
...
- Alle Funktionen verwenden einen blockierenden Ansatz (`HAL_UART_Transmit` /
  `HAL_UART_Receive`) mit einem Timeout von 1000 ms.
- Die Funktionen sind nur aktiv, wenn das Symbol `DEBUG_BASICUART` definiert ist.
  Dies ist standardmäßig der Fall, wenn `DEBUG` gesetzt ist. Für eine dauerhafte
  Aktivierung unabhängig vom Build-Typ kann `DEBUG_BASICUART` manuell in den
  Compiler-Präprozessordefinitionen gesetzt werden.
- `initBasicUart()` muss vor dem ersten Aufruf einer Sende- oder Empfangsfunktion
  aufgerufen werden.

## Abhängigkeiten

- `main.h` – STM32 HAL
- `usart.h` – generierte UART-Handle-Deklaration
